...
<script src="admin/js/jquery-1.10.2.min.js"></script>
 ...
 <body class="sticky-header left-side-collapsed"  onload="initMap()">
    ...
    <c:if test="${pageNumber gt 1}">
        <a href="Backup_details.jsp?pageNumber=${pageNumber - 1}">Previous</a>
    </c:if>
    <c:forEach begin="1" end="${numberOfPages}" var="i">
        <c:choose>
            <c:when test="${i!=pageNumber}">
                <a href="Backup_details.jsp?pageNumber=<c:out value="${i}"/>"><c:out value="${i}"/></a>
            </c:when>
            <c:otherwise>
                <c:out value="${i}"/>
            </c:otherwise>        
    ...
    <c:if test="${pageNumber lt numberOfPages}">
        <a href="Backup_details.jsp?pageNumber=${pageNumber + 1}">Next</a>
    </c:if>
			</div>
			</div>
   </section>
